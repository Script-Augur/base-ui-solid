...
export function resolveThumbCollision(
  ...
  currentValues: ReadonlyArray<number> | null | undefined,
  initialValues: ReadonlyArray<number> | null | undefined,
  pressedIndex: number,
  nextValue: number,
  min: number,
  max: number,
  ...
  switch (behavior) {
    case 'swap': {
      ...
      if (shouldSwapForward) {
        nextValueForTarget = Math.max(
          nextValue,
          candidateValues[targetIndex]!
        )
      } else {
        nextValueForTarget = Math.min(
          nextValue,
          candidateValues[targetIndex]!
        )
      }
      ...
      neighborUpperBound = Math.min(
        neighborUpperBound,
        max -
          (adjustedValues.length - 1 - neighborIndex) * minValueDifference
      )
