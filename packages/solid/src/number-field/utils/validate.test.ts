...
describe('NumberField validate', () => {
  describe('removeFloatingPointErrors', () => {
    ...
    it('preserves safe integers exactly without a format', () => {
      expect(removeFloatingPointErrors(Number.MAX_SAFE_INTEGER)).toBe(
        Number.MAX_SAFE_INTEGER
      )
      expect(removeFloatingPointErrors(Number.MIN_SAFE_INTEGER)).toBe(
    ...
    it('rounds compact notation against its standard equivalent', () => {
      ...
      expect(
        removeFloatingPointErrors(1234.567, {
          notation: 'compact',
    ...
    it('returns 0.3 for 0.2 + 0.1 with maximumFractionDigits', () => {
      expect(
        removeFloatingPointErrors(0.2 + 0.1, { maximumFractionDigits: 1 })
      ).toBe(0.3)
    })
    ...
    it('preserves tiny values when Intl supports more than 20 fraction digits', () => {
      expect(
        removeFloatingPointErrors(1e-21, { maximumFractionDigits: 21 })
      ).toBe(1e-21)
    })
    ...
    it('ignores formatting style', () => {
      expect(
        removeFloatingPointErrors(1000, { style: 'currency', currency: 'USD' })
      ).toBe(1000)
    })

    it('uses resolved maximumFractionDigits when only minimum is provided', () => {
      expect(
        removeFloatingPointErrors(1.234567, { minimumFractionDigits: 5 })
      ).toBe(1.23457)
    })
  })
  ...
  describe('toValidatedNumber', () => {
    ...
    it('preserves parsed input beyond 15 significant digits when step is undefined', () => {
      ...
      expect(
        toValidatedNumber(1.234567890123456, {
          ...defaultOptions,
          step: undefined,
        })
      ).toBe(1.234567890123456)
      expect(
        toValidatedNumber(0.1234567890123456, {
          ...defaultOptions,
          step: undefined,
        })
    ...
    it('preserves large fractional values when stepping cleanup would be too coarse', () => {
      ...
      expect(
        toValidatedNumber(steppedValue, {
          ...defaultOptions,
          step: 0.1,
          snapOnStep: false,
        })
...
function toValidatedNumber(value: number | null, options: ValidateOptions) {
  return toValidatedNumberImpl(
    value,
    options.step,
...
// Adapter option shape for the positional `toValidatedNumber` helper above.
