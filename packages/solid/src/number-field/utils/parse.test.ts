...
const isJSDOM =
  typeof navigator !== 'undefined' && /jsdom|happy/i.test(navigator.userAgent)
...
describe('NumberField parse', () => {
  ...
  describe('parseNumber', () => {
    ...
    it('handles percentages with style: "unit" and unit: "percent"', () => {
      expect(
        parseNumber('12%', 'en-US', { style: 'unit', unit: 'percent' })
      ).toBe(12)
    })
    ...
    it('strips an interleaved percent sign (1%2)', () => {
      ...
      expect(
        parseNumber('1%2', 'en-US', { style: 'unit', unit: 'percent' })
      ).toBe(12)
    })
    ...
    it('parses units when options specify unit style', () => {
      expect(
        parseNumber('12 kg', 'en-US', { style: 'unit', unit: 'kilogram' })
      ).toBe(12)
    })
    ...
    it('parses currency prefix/suffix across locales', () => {
      ...
      expect(
        parseNumber(en, 'en-US', { style: 'currency', currency: 'EUR' })
      ).toBe(1234.56)
      expect(
        parseNumber(fr, 'fr-FR', { style: 'currency', currency: 'EUR' })
      ).toBe(1234.56)
    })

    it('parses units with different formats', () => {
      expect(
        parseNumber('12 km/h', 'en-US', {
          style: 'unit',
          unit: 'kilometer-per-hour',
        })
      ).toBe(12)
      expect(
        parseNumber('12 m/s', 'en-US', {
          style: 'unit',
