import { getFormatter } from '@script-augur/base-ui-utils'
...
export function parseNumber(
  ...
  const unitRegex = unitParts.length
    ? new RegExp(unitParts.join('|'), 'g')
    : null
  ...
  const replacements: Array<[RegExp | null, string | ((m: string) => string)]> =
    [
      ...
      [
        exponentSeparator
          ? new RegExp(escapeRegExp(exponentSeparator), 'g')
          : null,
        'e',
      ],
      ...
      [
        HAN_RE,
        (ch: string) => String(Math.max(HAN_NUMERALS.indexOf(ch) - 1, 0)),
      ],
    ]
  ...
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- set in replace callback
  ...
  const hasPercentSymbol =
    PERCENT_RE.test(formattedNumber) || style === 'percent'
