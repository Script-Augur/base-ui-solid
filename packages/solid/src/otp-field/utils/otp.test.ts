...
describe('otp utils', () => {
  ...
  it('applies custom normalization after built-in validation', () => {
    ...
    expect(
      normalizeOTPValue('ab-12 cd!', 6, 'alphanumeric', normalizeValue)
    ).toBe('AB12CD')
  ...
  it('filters custom normalization output through built-in validation', () => {
    expect(normalizeOTPValue('12', 6, 'numeric', value => `${value}AB`)).toBe(
      '12'
    )
  })
  ...
  it('clamps values after custom normalization', () => {
    expect(
      normalizeOTPValue('123456', 4, 'numeric', value => `${value}789`)
    ).toBe('1234')
  })
  ...
  it('applies custom normalization when replacing OTP values', () => {
    ...
    expect(
      replaceOTPValue('123456', 2, 'ab', 6, 'alphanumeric', normalizeValue)
    ).toBe('12AB56')
  })

  it('preserves suffix characters when custom normalization removes part of a middle replacement', () => {
    expect(
      replaceOTPValue('1303', 1, '29', 4, 'numeric', value =>
        value.replace(/[^0-3]/g, '')
      )
    ).toBe('1203')
  })
